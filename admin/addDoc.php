...
//__variables pour les balises méta
    ...
    try {
		if (isset($_POST) && isset($_POST['iddoc']) && isset($_POST['region']) && isset($_POST['date']) && isset($_POST['siecle']) && isset($_POST['typo']) && isset($_POST['transcription']) && isset($_POST['lat']) && isset($_POST['long'])){
			extract($_POST);
		
			if (!empty($iddoc) && !empty($region) && !empty($authord) && !empty($transcripteur) && !empty($date) && !empty($siecle) && !empty($typo) && !empty($image) && !empty($transcription) && !empty($lat) && !empty($long)){
				$iddoc = $Document->verifData($iddoc);
				$region = $Document->verifData($region);
				$region = $Document->verifData($authord);
				$region = $Document->verifData($transcripteur);
				...
				$data->createD($iddoc, $region, $authord, $transcripteur, $date, $siecle, $typo, $image, $transcription, $lat, $long);
